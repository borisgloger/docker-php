...
for version in "${versions[@]}"; do
	...
	docker build -f ${file} --tag "borisgloger/php-fpm:${version}" ${directory}/
done
...
for version in "${versions[@]}"; do
	...
	cat <<- DOCKERFILE > ${file}
		# Beware: This file is generated by the generate.sh script!
		FROM borisgloger/php-fpm:${version}
		EXPOSE 80
	...
	docker build -f ${file} --tag "borisgloger/php-nginx:${version}" ${directory}/
done
